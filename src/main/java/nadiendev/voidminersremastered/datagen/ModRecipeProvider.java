...
public class ModRecipeProvider extends RecipeProvider {
    ...
    private static void saveGem(CrystalSet set, int tier, float weight, ResourceKey<Level> dimension, RecipeOutput output) {
        MinerRecipe.Builder.builder(
                new WeightedStack(new ItemStack(set.CRYSTAL.get(), GEM_OUTPUT_PER_TIER[tier - 1]), weight), tier, false, dimension
        ).save(output);
    }
    ...
    protected void buildRecipes(RecipeOutput pWriter) {
        ...
        for (int i = 0; i < gemSets.size(); i++) {
            ...
            for (int tier = gemTier; tier <= GEM_OUTPUT_PER_TIER.length; tier++) {
                saveGem(set, tier, GEM_WEIGHT_OVERWORLD, Level.OVERWORLD, pWriter);
                saveGem(set, tier, GEM_WEIGHT_NETHER, Level.NETHER, pWriter);
                saveGem(set, tier, GEM_WEIGHT_MINING, MINING, miningWriter);
            }
        }
